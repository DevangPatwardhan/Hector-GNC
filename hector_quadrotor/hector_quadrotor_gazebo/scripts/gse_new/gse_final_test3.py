...
class MarkerPublisher:
    ...
    def publish_vector_between_points(self, point1, point2):
        ...
        vector_marker.color.r = 1.0  # Red color
        vector_marker.color.g = 1.0  # Green color
        vector_marker.color.b = 0.0  # Blue color (set to 0 for yellow)
        vector_marker.color.a = 1.0  # Alpha (transparency)
...
class gse:
    ...
    def shape(self,p,x,xobs ):# p- random point , xnearest- nearest point , xbos obstacle list
        ...
        for index, value in enumerate(fi):
            ...
            if value == 2:
                print(f"Value 2 found at index {index}")
                global shape_no
                shape_no=index
                break
    ...
    def shape_classifier(self,p,x,xobs,nix, theta_i):
        ...
        if (magnitude_A_nix * magnitude_B_xp)==0:
    ...
    def xobstacle(self,x):
        ...
        for i in range(len(xobs)):
            ...
            #xobs_dict[key] = [rix-offset[i], pix,lix,nix]
            #xobs_dict[key] = [rix-offset[i], pix,offset[i],nix]
...
def random_point_in_cone_biased(A, V, h, R):
    Ax, Ay, Az = A
    Vx, Vy, Vz = V
    
    # Calculate the direction vector from apex to vertex
    ...
    d_norm = (dx / length, dy / length, dz / length)
    
    # Choose a random height z within the cone's height, biased towards the apex
    u = random.uniform(0, 1)
    z = h * (1 - math.sqrt(u))  # Skewing towards 0 (apex)
    
    # Calculate the radius at height z
    r = R * (z / h)
    
    # Generate random values for radius scaling and angle
    u1 = random.uniform(0, 1)
    u2 = random.uniform(0, 1)
    
    r_rand = r * math.sqrt(u1)
    theta_rand = 2 * math.pi * u2
    
    # Convert the polar coordinates to Cartesian coordinates in the base plane
    x_base = r_rand * math.cos(theta_rand)
    y_base = r_rand * math.sin(theta_rand)
    
    # Calculate the position of the base point at height z along the direction vector
    P_base = (Ax + z * d_norm[0], Ay + z * d_norm[1], Az + z * d_norm[2])
    
    # Translate the base coordinates to the 3D coordinates
    Px = P_base[0] + x_base
    Py = P_base[1] + y_base
    Pz = P_base[2]
...
def main():
    try:
        ...
        while not rospy.is_shutdown():
            ...
            #r1 = float(input("Enter random point x: "))
            #r2= float(input("Enter random point y: "))
            #r3 = float(input("Enter random point z: "))
            random_point = Point()
            ...
            gi_value,else_fi= gse_sample.shape(random_point,xnearest,xobs )
            print("gi value:",gi_value)
            ...
            if (gi_value==0):
                ...
                global key_id,first_safezone_shape
                key_id = f"xobs{else_fi}"
                first_safezone_shape = xobs[key_id]
                print("first_safezone_shape",first_safezone_shape) 
                        
                point_inside_first_shape=random_point_in_cone_biased(first_nearest_coords, first_safezone_shape[2], first_safezone_shape[1], first_safezone_shape[3])
                steer(first_nearest_vertex,second_nearest_vertex,first_nearest_coords,second_nearest_coords,point_inside_first_shape,g, marker_pub,node_name,i)
                #break
