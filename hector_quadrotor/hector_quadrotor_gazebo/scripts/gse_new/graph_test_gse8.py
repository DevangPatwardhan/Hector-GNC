...
def add_vertex(graph, marker_pub):
    vertex_name = input("Enter vertex name: ")
    x = float(input("Enter x coordinate for vertex: "))
    y = float(input("Enter y coordinate for vertex: "))
    z = float(input("Enter z coordinate for vertex: "))
    r = 0
    g_value = 0
    b = 1
    a = 1
...
def add_edge(graph, marker_pub, vertex_name):
    if len(graph.vertices) < 2:
        print("At least two vertices are required to add an edge.")
        return
    
    first_nearest_vertex = input("Enter the first vertex of the edge: ")
    second_nearest_vertex = input("Enter the second vertex of the edge: ")
    
    edge_cost_to_first_vertex = float(input(f"Enter edge cost to connect {vertex_name} to {first_nearest_vertex}: "))
    edge_cost_to_second_vertex = float(input(f"Enter edge cost to connect {vertex_name} to {second_nearest_vertex}: "))
    
    
    graph.add_edge(vertex_name, first_nearest_vertex, edge_cost_to_first_vertex)
    graph.add_edge(vertex_name, second_nearest_vertex, edge_cost_to_second_vertex)
...
if __name__ == "__main__":
    try:
        ...
        print("-----------------------------------------------")
        print("graph for nearest intersection point :")
