...
class gse:
    ...
    def shape_obstacle_iterator(self,p,x,xobs ):
        ...
        for i in range(len(xobs)):
            ...
            (angle_difference[i],ri[i],sat_fi_angle[i],sat_fi_distance[i])=self.shape_classifier(p,x,pix_point_point,theta_i )
    ...
    def shape_classifier(self,p,x,xobs, theta_i):
        ...
        # Calculate ri
        angle_difference = (theta_radians - theta_i)#radians
        #angle_difference = (theta_degrees - theta_i)#angle
        #ri = np.arccos((dot_product) / ((magnitude_ni_X) * (magnitude_V_i_X))) - (theta_i)
        print("theta_radians:",theta_radians)
