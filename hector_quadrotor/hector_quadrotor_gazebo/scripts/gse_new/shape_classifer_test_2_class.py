...
def shape_check(xnearest,pix_list,random_point ,marker_pub ):
    ...
    (ri,sat_ri,sat_fi,sat_fi_distance)=shape(xnearest,pix_list,random_point ,marker_pub )
    sat_ri_int = [int(value) for value in sat_ri]
    sat_fi_int = [int(value) for value in sat_fi]
    sat_fi_distance_int = [int(value) for value in sat_fi_distance]
     

    # Add corresponding integer values and store in a new list
    sat_fi_org = [int(a) + int(b) for a, b in zip(sat_fi_int, sat_fi_distance_int)]
    ...
    if all(sat_ri):
        ...
        #print("len(sat_ri_int)",len(sat_ri_int))
        #print("len(sat_ri_int)",len(sat_fi_org)) 
        #print("len(sat_ri_int)",len(sat_ri_int))   
        gi = []  # Initialize the list for g_i values
        for i in range(1, len(sat_fi_org) + 1):
            
            # Calculate the summation part for each i
            summation = sum(sat_ri_int[:i-1])
            # Calculate g_i and append to the list
            #gi.append(sat_ri_int[i-1] + summation - (i + 1))
            gi.append(sat_fi_org[i-1] + summation - (i + 1))
            print("gi list",gi)
...
def shape(xnearest,pix_list,random_point ,marker_pub ):
    ...
    for i in range(len(pix_list)):
        ...
        print("pix_list[i]",pix_list[i])
        first_element = pix_list[i]
        print(first_element)
        print(first_element[0])
        print(first_element[1])
        print(first_element[2])
        pix_point_point = Point()
        pix_point_point.x = (first_element[0] -0)
        pix_point_point.y = (first_element[1] -0)
        pix_point_point.z = (first_element[2] -0)
        ri_X = calculate_distance(xnearest, pix_point_point)
        ...
        (ri[i],sat_ri[i],sat_fi[i],sat_fi_distance[i])=shape_classifier(xnearest,pix_point_point, random_point , theta_i)
        marker = publish_marker(xnearest, pix_point_point, height, radius)
        marker_pub.publish(marker)
        time.sleep(3)
...
if __name__ == "__main__":
    ...
    #shape(start_point, pix_list,o_random_point ,marker_pub )
   
    gi_value=shape_check(start_point, pix_list,o_random_point ,marker_pub )
    ...
    #marker = publish_marker(point, pix_point1, height, radius)
    
    #marker = publish_marker(point, pix_point2, height, radius)

    # Publish the marker
    while not rospy.is_shutdown():
        publish_axes(marker_pub)
        publish_single_point(o_random_point)
        publish_points(point, pix_point1, marker_pub)
        publish_points(point, pix_point2, marker_pub)
        #publish_vector_between_points(point, point2, marker_pub)
        #publish_vector_between_points(origin_point, origin_point3, marker_pub)
        publish_vector(point, pix_point1, marker_pub)
        publish_vector(point, pix_point2, marker_pub)
        #marker_pub.publish(marker)
        rospy.sleep(1)
