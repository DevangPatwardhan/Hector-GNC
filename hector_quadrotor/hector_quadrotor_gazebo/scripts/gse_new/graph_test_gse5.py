...
#def add_vertex(graph, marker_pub, x_rand, y_rand, z_rand):
def add_vertex(graph, marker_pub):
    vertex_name = input("Enter vertex name: ")
    #x = x_rand
    #y = y_rand
    #z = z_rand
    x = float(input("Enter x coordinate for vertex vertex: "))
    y = float(input("Enter y coordinate for vertex vertex: "))
    z = float(input("Enter z coordinate for vertex vertex: "))
    r = 0
    g_value = 0
    b = 1
    a = 1
    point=(x,y,z)
    near , distance = nearest(graph, point)
    print("Nearest point:", near)
    print("Nearest point distance:", distance)
    color = ColorRGBA(r, g_value, b, a)
    graph.add_vertex(vertex_name, x, y, z, color)
    update_markers(graph, None, None, None, marker_pub)
    return vertex_name

#def add_edge(graph, marker_pub, current_node, xnearest):
def add_edge(graph, marker_pub):
    if len(graph.vertices) < 2:
        print("At least two vertices are required to add an edge.")
        return
    #vertex1 = current_node
    #vertex2 = xnearest
    vertex1 = input("edge first vertex ")
    vertex2 = input("edge second vertex ")
...
def nearest(g, point):
    ...
    for vertex, (x, y, z, _) in g.vertices.items():
        ...
        if distance < min_distance:
            ...
            nearest_coords = (x, y, z)

    return nearest_vertex,nearest_coords
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            #z = float(input("Enter z coordinate for goal vertex: "))
            #point=(x,y,z)
            #near = nearest(g, point)
            #print("Nearest point:", near)
            #add_vertex(g, marker_pub, point[0], point[1], point[2])
            #add_edge(g, marker_pub, near, point)
            add_vertex(g, marker_pub)
            add_edge(g, marker_pub)
