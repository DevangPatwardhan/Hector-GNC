...
def check_collision(points, shape):
    colliding_points = []
    for point in points:
        distance = math.sqrt((point.x - shape.pose.position.x) ** 2 + (point.y - shape.pose.position.y) ** 2 + (point.z - shape.pose.position.z) ** 2)
        if distance <= shape.scale.x / 2:  # Assuming shape.scale.x represents the diameter
...
def main():
    ...
    try:
        ...
        while not rospy.is_shutdown():
            points = generate_uniform_points(num_points_x, num_points_y, num_points_z, distance, marker_pub)
            shape = publish_shape(height, radius, marker_pub)
            colliding_points = check_collision(points, shape)
            print("Colliding points indices:", [points.index(p) for p in colliding_points])
            rate.sleep()
