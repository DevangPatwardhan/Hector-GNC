...
class ObstacleDetection:
    ...
    def pointcloud_callback(self, msg):
        ...
        for label in unique_labels:
            ...
            self.publish_center_outside_breadth(marker_array, bbox, marker_id)
            marker_id += 1
    ...
    def publish_center_outside_breadth(self, marker_array, bbox, marker_id, offset=0.1):
        center = bbox.get_center()
        extents = bbox.get_extent()
        
        # Calculate the point outside the cuboid along the breadth direction
        outside_point = np.array([center[0], center[1] + (extents[1] / 2.0) + offset, center[2]])
        ...
        point_marker.type = Marker.SPHERE
        point_marker.action = Marker.ADD
        point_marker.pose.position.x = outside_point[0]
        point_marker.pose.position.y = outside_point[1]
        point_marker.pose.position.z = outside_point[2]
        point_marker.pose.orientation.w = 1.0
        point_marker.scale.x = 0.1  # Size of the point marker
        point_marker.scale.y = 0.1
        point_marker.scale.z = 0.1
