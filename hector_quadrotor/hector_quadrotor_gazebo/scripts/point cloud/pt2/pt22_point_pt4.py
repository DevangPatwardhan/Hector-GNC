...
def publish_tracked_obstacles():
    ...
    for track in track_list:
        ...
        # Create text marker for obstacle ID
        text_marker = Marker()
        text_marker.header.frame_id = "world"
        text_marker.header.stamp = rospy.Time.now()
        text_marker.ns = "tracked_obstacles_text"
        text_marker.id = track.id  # Use the same ID as the obstacle marker
        text_marker.type = Marker.TEXT_VIEW_FACING
        text_marker.action = Marker.ADD
        text_marker.pose.position.x = center[0]
        text_marker.pose.position.y = center[1]
        text_marker.pose.position.z = center[2] + extents[2] + 0.1  # Adjust height for visibility
        text_marker.pose.orientation.w = 1.0
        text_marker.scale.z = 0.05  # Text thickness
        text_marker.color.r = 1.0
        text_marker.color.g = 1.0
        text_marker.color.b = 1.0
        text_marker.color.a = 1.0
        text_marker.text = f"Obstacle {track.id}"  # Text to display (obstacle ID)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        rospy.loginfo("Obstacles info: %s", obstacles_info)
