...
def turtlesim_semicircle():
    ...
    while True:
        print("fn2")
        try:
            #eading_str = raw_input("Enter initial heading (radians, -1.57 to 1.57): ")
            initial_heading = float(-1.57)
            if -1.57 <= initial_heading <= 1.57:
                break
            else:
                print("Invalid heading. Please enter a value between -1.57 and 1.57.")
        except ValueError:
            print("Invalid input. Please enter a valid float value.")
    print("fn3")
    # Calculate linear and angular velocities based on the formula
    linear_velocity = radius / 2.0
    angular_velocity = -(linear_velocity / radius)

    # Create a Twist message to move the turtle in a semicircle
    twist = Twist()
    twist.linear.x = linear_velocity
    twist.angular.z = angular_velocity

    # Duration of the movement (adjust as needed)
    duration = 3.05 * radius / linear_velocity

    # Keep the turtle moving for the specified duration
    start_time = rospy.Time.now().to_sec()
    while (rospy.Time.now().to_sec() - start_time) < duration:
        sender.publish(twist)

        # Optionally, adjust heading gradually during the movement
        # for smoother turning (if needed)
        semicircle_smooth = True
        if semicircle_smooth:
            current_time = rospy.Time.now().to_sec()
            elapsed_time = current_time - start_time 
            print("fn4")
            new_heading = initial_heading + elapsed_time * angular_velocity * 0.5
            if new_heading > 1.57:  # Limit heading to +/- 90 degrees
                new_heading = 1.57
            elif new_heading < -1.57:
                new_heading = -1.57
...
def move_in_semircle_cw():
        pubb = rospy.Publisher('/cmd_vel', Twist, queue_size=10)
        linear_speed = 5  # m/s
        angular_speed = 0.5 # rad/s
        move_duration = 4.0  # seconds
        angular_duration = pi  # radians (180 degrees)
        twist = Twist()
        start_time = rospy.Time.now()
        while rospy.Time.now() - start_time < rospy.Duration(angular_duration / angular_speed):
...
def stop():
    pubbb = rospy.Publisher('/cmd_vel', Twist, queue_size=10)
