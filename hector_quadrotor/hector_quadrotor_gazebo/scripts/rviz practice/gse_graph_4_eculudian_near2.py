...
def publish_graph(g, start, goal, shortest_path):
    ...
    for idx, (vertex_name, (x, y, z, color)) in enumerate(g.vertices.items()):
        ...
        text_marker.pose.position = Point(x=x, y=y, z=z + 0.2)  # Adjust for text position
...
def steer_to_nearest_vertex(graph, temp_vertex):
    min_distance = float('inf')
    nearest_vertex = None

    for vertex, (x, y, z, _) in graph.vertices.items():
        distance = euclidean_distance(temp_vertex, (x, y, z))
...
if __name__ == "__main__":
    try:
        ...
        # Temporary vertex (steer function)
        temp_vertex_name = input("\nEnter temporary vertex name: ")
        temp_x = float(input("Enter x coordinate: "))
        temp_y = float(input("Enter y coordinate: "))
        temp_z = float(input("Enter z coordinate: "))
        temp_vertex = (temp_x, temp_y, temp_z)
        nearest_vertex, distance = steer_to_nearest_vertex(g, temp_vertex)
        print(f"The nearest vertex to {temp_vertex_name} is {nearest_vertex} at a distance of {distance:.2f}.")
