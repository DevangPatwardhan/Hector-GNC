...
class MazebotGTG:
    def __init__(self):
        ...
        self.azimuth_to_goal = 0.0
        self.elevation_to_goal = 0.0
        self.angular_velocity_scale = 2
        self.linear_velocity_scale = 0.5
        self.vertical_velocity_scale = 0.5
    ...
    def goal_movement(self):
        ...
        while not rospy.is_shutdown():
            ...
            K_vertical = self.vertical_velocity_scale
            ...
            # Calculate elevation to goal
            self.elevation_to_goal = atan2(self.goal_pose.z - self.robot_pose.z, 
                                           sqrt((self.goal_pose.x - self.robot_pose.x)**2 + 
                                                (self.goal_pose.y - self.robot_pose.y)**2))
            ...
            linear_speed = K_linear * sqrt((self.goal_pose.x - self.robot_pose.x)**2 + (self.goal_pose.y - self.robot_pose.y)**2)
            #linear_speed = 1
            vertical_speed = K_vertical * (self.goal_pose.z - self.robot_pose.z)

            self.vel_msg.linear.x = linear_speed * cos(self.elevation_to_goal)
            self.vel_msg.linear.y = linear_speed * sin(self.elevation_to_goal)
            self.vel_msg.linear.z = vertical_speed
