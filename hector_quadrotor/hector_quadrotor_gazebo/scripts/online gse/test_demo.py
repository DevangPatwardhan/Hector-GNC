...
class gse:
    ...
    def get_center_outside(self,random_pt, center):
        offset=0.5
        # Calculate the direction vector from the robot to the center of the obstacle
        print("center:",center)
        np_random_pt = np.array([random_pt.x, random_pt.y, random_pt.z])
        np_center = np.array([center.x, center.y, center.z])
        direction_vector = np_center - np_random_pt
        direction_vector /= np.linalg.norm(direction_vector)
        ...
        # Calculate the outside point
        outside_point = np_center - direction_vector * offset
    ...
    def xobstacle_near_point(self,x,obstacles_info,random_pt):
        ...
        for value in center:
            ...
            xobs1.append(self.get_center_outside(random_pt, center_tf))
        ...
        for i in range(len(xobs)):
            ...
            #xobs_dict[key] = [rix, pix,lix,nix]
            #xobs_dict[key] = [rix, pix,offset[i],nix]
            xobs_dict1[key] = [rix, pix,offset[i],nix]
        ...
        while not rospy.is_shutdown():
            ...
            if (obstacles_info==0):
                ...
                while True:
                    ...
                    obstacles_info1 = get_obstacles_info()
                    #xobs,xobs1=gse_sample.xobstacle(xnearest,obstacles_info1)
                    xobs,xobs1=gse_sample.xobstacle_near_point(xnearest,obstacles_info1,random_point)
                    #xobstacle_near_point(self,x,obstacles_info,random_pt)
                    ...
                    indicator=1    
                    #gi_value_goal= gse_sample.shape(goal,random_point,xobs1,indicator )
                    gi_value_goal= gse_sample.shape(random_point,goal,xobs1,indicator )
                    print("gi_value_goal:",gi_value_goal)
                    input("Press Enter to continue...")    
            
                    if (gi_value==0) and (gi_value_goal==0):
                        
                            
                        print("random point is in safezone from current position")
                        print("goal is in safezone from random point")
                            
                        obs1_values = (random_x,random_y,pos_z) 
                        waypoints[node_name] = obs1_values
                        print("waypoints:",waypoints)
                        
                        distance_goal1 = euclidean_distance(point_a, point_b)
