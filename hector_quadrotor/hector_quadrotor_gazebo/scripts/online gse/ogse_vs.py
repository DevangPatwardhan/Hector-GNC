...
class gse:
    ...
    def shape(self,p,x,xobs ,indicator ):# p- random point , xnearest- nearest point , xbos obstacle list
        ...
        (angle_difference,ri,sat_fi_angle,sat_fi_distance)=self.shape_obstacle_iterator(p,x,xobs,indicator )
        #print("fi:",fi)
        print("ri:",ri)
    ...
    def shape_obstacle_iterator(self,p,x,xobs,indicator):
...
def generate_random_point_in_spherical_sector(apex, rix, phi_a, d_phi, theta_a, d_theta):
    ...
    return Point(translated_x, translated_y, translated_z)
    ...
    def goal_movement(self,goal_x,goal_y):
        ...
        while not rospy.is_shutdown():
            ...
            if abs(angle_difference) > 0.1:  # Some small threshold to start moving
                ...
                self.vel_msg.linear.x = 1  # Constant linear velocity
...
def euclidean_distance_3axis(point1, point2):
...
def main():
    try:
        ...
        i=0
        j=0
        goal_x = float(input("Enter goal_pose.x: "))
        goal_y = float(input("Enter goal_pose.y: "))
        while not rospy.is_shutdown():
            i=i+1
            ...
            marker_publisher.publish_marker(goal_x,goal_y,pos_z)
            
            start_color = ColorRGBA(0, 0, 1, 1)
            node_name=f"node{{{i} {j}}}"
            g.add_vertex("goal", goal_x, goal_y, pos_z, start_color)
            g.add_vertex(node_name, pos_x, pos_y, pos_z, start_color)
            
            temporary_graph.add_vertex("goal", goal_x, goal_y, pos_z, start_color)
            temporary_graph.add_vertex(node_name, pos_x, pos_y, pos_z, start_color)
            
            
            
            point_a = (pos_x, pos_y)
            point_b = (goal_x, goal_y)
            
            #reached=mazebot_gtg.goal_movement(goal_x,goal_y)
            ...
            rospy.sleep(2)  # Give some time to receive the messages
            ...
            if (obstacles_info==0):
                ...
                random_x,random_y,random_z=gse_sample.generate_point_towards_goal(pos_x,pos_y,pos_z, goal_x, goal_y, pos_z)
                
                goal_point = Point()
                goal_point.x = (random_x)
                goal_point.y = (random_y)
                goal_point.z = (pos_z)
                #marker_publisher.publish_single_point(goal_point)
                lix=3.5
                phi_a, d_phi, theta_a, d_theta=calculate_spherical_sector_angles(robot_pos, goal_point, lix)
                
                rix = euclidean_distance_3axis(robot_pos, goal_point)
                random_point=generate_random_point_in_spherical_sector(robot_pos,rix, phi_a, d_phi, theta_a, d_theta)
                marker_publisher.publish_single_point(random_point) 
                frame_id = "world"
                orientation, slant_height, hsc, rsc, rix_c, half_angle, rcb, hc = marker_publisher.calculate_orientation_and_geometry(robot_pos, goal_point, lix)
                cone_marker = marker_publisher.create_cone_marker(frame_id, 0, robot_pos, orientation, hc, rcb)
                cap_marker = marker_publisher.create_spherical_cap_marker(frame_id, 1, robot_pos, orientation, slant_height, hsc, rsc, rix_c)
                #marker = shape_publisher.publish_marker(x, pix_point_point, height, radius)
                marker_publisher.marker_pub1.publish(cone_marker)
                marker_publisher.marker_pub1.publish(cap_marker)
                   
                obs1_values = (random_point.x,random_point.y,random_point.z) 
                waypoints[node_name] = obs1_values
                print("waypoints:",waypoints)
                
                                
                reached=mazebot_gtg.goal_movement(random_point.x,random_point.y)
                if (reached==0):
                ...
                if distance_goal<=5:
                    ...
                    reached_goal=mazebot_gtg.goal_movement(goal_x,goal_y)
                    if (reached_goal==0):
                ...
                while True:
                    ...
                    random_point.z = (pos_z)
            
                    #marker_publisher.publish_marker(random_x,random_y,random_z)
                    #marker_publisher.publish_single_point(random_point)
                    ...
                    obstacles_info1 = get_obstacles_info()
                    xobs=gse_sample.xobstacle(xnearest,obstacles_info1)
                    
                    indicator=1   
                    gi_value= gse_sample.shape(random_point,xnearest,xobs,indicator )
                    print("gi value:",gi_value)
                    indicator=1    
                    gi_value_goal= gse_sample.shape(goal,xnearest,xobs,indicator )
                    print("gi_value_goal:",gi_value_goal)
                    
                    
                    #if (0):
                    if (gi_value_goal==0):
                        print("--------------------------------------gi_value_goal==0--------------------------------------------")
                        print(" ")
                        print(" ")
                        print(" ")
                        print(" ") 
                        ...
                        obs1_values = (random_x,random_y,pos_z) 
                        waypoints[node_name] = obs1_values
                        print("waypoints:",waypoints)
                        random_x,random_y,random_z=gse_sample.generate_point_towards_goal(pos_x,pos_y,pos_z, goal_x, goal_y, pos_z)
                        
                        random_point1 = Point()
                        random_point1.x = (random_x)
                        random_point1.y = (random_y)
                        random_point1.z = (pos_z)
            
                        #marker_publisher.publish_marker(random_x,random_y,random_z)
                        marker_publisher.publish_single_point(random_point1)
