...
def process_pointcloud(msg):
    ...
    for label in unique_labels:
        ...
        extents[0]=(extents[0]+0.5)#changed
        extents[2]=(extents[2]+0.5)#changed
...
class gse:
    ...
    def xobstacle(self,x,obstacles_info):
        ...
        for i in range(len(xobs)):
            ...
            xobs_dict[key] = [rix, pix,(offset[i]+0.2),nix]
    ...
    def xobstacle_near_point(self,x,obstacles_info,random_pt,goal):
        ...
        for i in range(len(xobs)):
            ...
            #xobs_dict[key] = [rix, pix,lix,nix]
            #xobs_dict[key] = [rix, pix,offset[i],nix]
            xobs_dict1[key] = [rix, pix,(offset[i]+0.2),nix]
...
def generate_random_point_in_spherical_sector(apex, rix, phi_a, d_phi, theta_a, d_theta):
    ...
    return Point(translated_x, translated_y, apex.z)
        ...
        while not rospy.is_shutdown():
            ...
            if (obstacles_info==0):
                ...
                while True:
                    ...
                    xrand=generatesample()#not Point()
                    random_point = Point()
                    ...
                    if gi_value==0:
                        ...
                        print("inside else")
                        #time.sleep(2)
                        items = list(xobs.items())

                        # Access the last tuple
                        last_key, last_value = items[-1]

                        print("Last key:", last_key)
                        print("Last value:", last_value)
                        last_obs = {last_key: last_value}
                        #xobs_dict[key] = [rix, pix,offset[i],nix]
                        X = Point(pos_x,pos_y,pos_z)
                        phi_a, d_phi, theta_a, d_theta = calculate_spherical_sector_angles(xnearest, last_value[1], last_value[2])
                        random_point_1 = generate_random_point_in_spherical_sector(X,last_value[0], phi_a, d_phi, theta_a, d_theta)
                        marker_publisher.publish_single_point(random_point_1)
                        print("------------------------------------------")
                        print("cone")
                        #time.sleep(5)
                        random_point1_list=[random_point_1.x,random_point_1.y,random_point_1.z]
                        first_nearest_vertex1, first_nearest_coords1, second_nearest_vertex1, second_nearest_coords1 =nearest(g,random_point1_list)
                        xnearest1=Point()
                        xnearest1.x=first_nearest_coords1[0]
                        xnearest1.y=first_nearest_coords1[1]
                        xnearest1.z=first_nearest_coords1[2]
                        #gi_value_1= gse_sample.shape(random_point_1,xnearest1,xobs )
                        gi_value_1= gse_sample.shape(random_point_1,X,xobs )
                        print("gi value of inside cone :",gi_value_1)
                        #time.sleep(5)
                        
                        if gi_value_1==0:
                            
                            print("gi value cone :is ZERO")
                            #steer(first_nearest_coords,xrand,g, marker_pub,node_name)
                            steer(first_nearest_vertex1,second_nearest_vertex1,first_nearest_coords1,second_nearest_coords1,random_point1_list,g, marker_pub,node_name,i)
                        
                            f=get_vertex_positions(g)
                            print("f",f)
                        
                            #nearest_intersected_shape(robot_pose_xnear,obstacles_info,f,random_point,xobs,gse_sample,g,marker_pub,node_name,xrand)
                            nearest_intersected_shape(robot_pose_xnear,obstacles_info,f,random_point_1,xobs,gse_sample,g,marker_pub,node_name,random_point1_list)
                    a=input("gse while exit press enter to contiue ")    
